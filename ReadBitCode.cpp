...
void graph_basicblock_print(BasicBlock& bb){
	...
	/*
	int fd = open("graph.dot", O_WRONLY, O_APPEND);
	if(fd == 0){
		std::cout << "FILE ERROR: graph.dot" << std::endl;
	}

	raw_ostream *os = new raw_fd_ostream(fd, true);
	bb.print(*os, false);
...
int main(int argc, char *argv[]) {
	...
	/*
	int fd = open("graph.dot", O_RDWR, 644);
	if(fd == 0){
		std::cout << "FILE ERROR: graph.dot" << std::endl;
	}
	
	//std::ofstream graph_out(fd);
	raw_ostream *os = new raw_fd_ostream(fd, true);
	*/
